 ...
 * 能够将bean的描述信息（注解）转换为BeanDefinition，
 * 并注册到DefaultListableBeanFactory的beanDefinitionMap
 */
public class AnnotatedBeanDefinitionReader {
	 ...
	 * @since 5.0
	 * 可以处理一些通用注解@Lazy，@Primary，@DependsOn，@Role，@Description
	 */
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		// 直接使用AnnotationConfigApplicationContext.register(Class<?>... componentClasses)
		// qualifiers默认为null
		if (qualifiers != null) {
		...
		// BeanDefinitionHolder是一个存放BeanDefinition的一个特殊的数据结构
